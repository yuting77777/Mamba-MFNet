...
class Dataset(data.Dataset): 
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        self.n_channels_A = opt.get('n_channels_A', 3)  # PET
        self.n_channels_B = opt.get('n_channels_B', 1)  # MRI
        self.patch_size = opt.get('H_size', 64)     # 使用get方法提供默认值
        self.sigma = opt.get('sigma', 25)            # 使用get方法提供默认值
        self.sigma_test = opt.get('sigma_test', self.sigma)  # 使用get方法提供默认值
    ...
    def __getitem__(self, index):
        ...
        img_A = util.imread_uint(A_path, self.n_channels_A)
        img_B = util.imread_uint(B_path, self.n_channels_B)
        ...
        if self.opt['phase'] == 'train': 
            ...
            """
            H, W, _ = img_A.shape
            # print("H 的 size是，",H) 480
            # print("W 的 size是，",W) 640
            # patch_size 是 128
            # --------------------------------
