...
class ModelPlain(ModelBase):
    ...
    def get_current_loss(self):
        """获取当前训练损失（假设使用 self.loss_G 作为主损失）"""
        if hasattr(self, 'loss_G'):
            return self.loss_G.item()
        elif hasattr(self, 'loss_total'):
            return self.loss_total.item()
        else:
            # 如果既没有 loss_G 也没有 loss_total，尝试从日志中提取
            logs = self.current_log()
            return logs.get('G_loss', logs.get('loss', 0))
